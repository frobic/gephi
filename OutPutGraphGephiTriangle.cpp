			...
			for (int k = j+1 ; k < Graph[i].size() ; k++) {
				if (i < Graph[i][j]) {
					if (isneigh(Graph[i][j],Graph[i][k])) {
						...
						Triangles[Graph[i][j]][Graph[i][k]]++ ;
						Triangles[Graph[i][k]][Graph[i][j]]++ ;
	...
	for (int i = 0 ; i < N ; i++) {	
		for (int j = 0 ; j < Graph[i].size() ; j++) {
			if (i < Graph[i][j] && MyComms[i].size() > 0 && MyComms[Graph[i][j]].size() > 0) {
				...
				w = (w < 0.25) ? 0.25 : w ;
				cout << "<edge id=\"" << k++ << "\" source=\""<< i <<"\" target=\"" << Graph[i][j] << "\" weight=\"" << w /*(double)(1.+intercomm(i,Graph[i][j]))/(double)(1.+unioncomm(i,Graph[i][j]))*/ << "\"/>" << endl ;
			}
		}
	}
